...
We propose Entropy-Gated Contrastive Decoding (EGCD), a routing framework with frozen weights and per-model threshold calibration, built around one observation: the conflict between visual evidence and language prior is visible in the first-token distribution before decoding starts. The router reads the Shannon entropy of that distribution. Low entropy means the model is confident and its priors are safe to trust. A sharp entropy spike marks vision-text conflict and activates a counterfactual contrastive pass that isolates the language prior and subtracts it, bounded by an Adaptive Plausibility Constraint. The same gate that helps biased architectures stays idle on structurally sound ones, which no uniform scheme can do. Our contributions are as follows:

\begin{enumerate}
\item An operationalisation of Visual Inertia \cite{gong2026attention} as a decoding-time signal. We take their attention-stagnation account and reduce it to a measurable probe based on the first-token distribution, where entropy separates correct from incorrect answers on every model (binary error-prediction AUC between 0.62 and 0.79; model-level values in Section~\ref{subsec:dynamics}). This motivates intervention at decoding time.

\item EGCD, a routing mechanism that uses first-token Shannon entropy as a real-time conflict probe and triggers counterfactual contrastive decoding only under active vision-text conflict. The gate adds no parameters, requires no sampling, and reads a statistic that the standard pass already computes. The threshold $\tau$ is calibrated on a small disjoint tuning split, with no weight updates.

\item A cross-architecture study on AMBER showing that contrastive interventions should be conditional: EGCD lifts balanced accuracy by up to 9.74 points on biased architectures and stays idle on structurally sound ones, avoiding the 10.62-point collapse that universal contrastive decoding inflicts on naturally balanced models.
\end{enumerate}
...
\caption{Overview of Entropy-Gated Contrastive Decoding (schematic). A relational query enters the frozen VLM; the first-token entropy $H(Y_{1})$ is compared against $\tau$, and the query is routed to System 1 (greedy decoding) or System 2 (counterfactual contrastive decoding with APC; $\alpha = 1.0$, $\beta = 0.1$). The selected-threshold interval across the seven evaluated models is $\tau^{*} \in [0.570, 0.700]$; the search grid is $[0.550, 0.750]$.}\label{fig:overview}
\end{figure}
...
Penalizing logits at every step taxes queries that need no help, so we treat decoding as a dual-process system \cite{kahneman2011thinking} and route once, at the query level. The probe is the Shannon entropy \cite{shannon1948mathematical} of the first-token distribution over the full vocabulary $V$:
...
A single threshold $\tau$ separates the two cases. Queries with $H(Y_{1}) < \tau$ are treated as settled: their priors can be trusted, and System 1 answers them with standard greedy decoding, giving $Y_{\text{std}}$. When $H(Y_{1}) \geq \tau$, the entropy spike is taken as a sign of vision-text conflict, and the query goes to the corrective System 2 pathway, which returns $Y_{\text{CD}}$. The gate reads a forward-pass statistic that System 1 already produces, so the routing check itself adds no forward pass; the extra cost (two expert forward passes per generated token of a routed query, one standard and one counterfactual) falls only on queries that actually route to System 2.
...
V_{\text{head}} \;=\; \bigl\{\, y \in V \,:\, P_{\text{std}}(y) \;\geq\; \beta\, \max_{w \in V} P_{\text{std}}(w)\,\bigr\}, \qquad 0 < \beta \leq 1.
\end{equation}
...
z_{\text{CD}}(y) \;=\; (1 + \alpha)\, z_{\text{std}}(y) \;-\; \alpha\, z_{\text{cf}}(y), \qquad \forall\, y \in V_{\text{head}}, \quad \alpha \geq 0.
\end{equation}
...
\State $z_{\text{std}} \leftarrow z_{\theta}(\,\cdot \mid X_{v}, X_{t})$; \; $P_{\text{std}} \leftarrow \text{softmax}(z_{\text{std}})$
...
\Else \Comment{System 2: counterfactual contrastive decoding}
  \While{$y \neq \langle\text{EOS}\rangle$ \textbf{and} $|Y| < T$}
    \State $z_{\text{std}} \leftarrow z_{\theta}(\,\cdot \mid X_{v}, X_{t}, Y)$; \; $P_{\text{std}} \leftarrow \text{softmax}(z_{\text{std}})$
    \State $z_{\text{cf}} \leftarrow z_{\theta}(\,\cdot \mid X_{t}, Y)$ \Comment{counterfactual text-only pass; distinct KV cache}
...
We evaluate on the AMBER benchmark \cite{wang2023amber}, restricted to its Discriminative-Relation subset: 1,664 image-text pairs of binary Yes/No questions about spatial alignment and physical contact. Existence suites such as POPE \cite{li2023bevaluating} and captioning metrics such as CHAIR \cite{rohrbach2018object} rarely stress relational binding, and AMBER's authors report affirmative bias across mainstream models, with Yes-ratios exceeding No-ratios for nearly every evaluated system. The subset is balanced by construction at the dataset level. Relational verification requires shifting attention to secondary objects, which makes this subset a direct probe of the Visual Inertia hypothesis. Scoring well on this subset requires reading the relation out of the image, which is the property we need from a test bed.
...
The gate handles this distinction without any explicit instruction. Balanced models maintain compact first-token distributions, entropy rarely crosses $\tau^{*}$, and the gate does not activate. As a result, EGCD preserves 75.11\% on LLaVA-1.5-7B and 85.65\% on LLaVA-v1.6-Mistral-7B, matching greedy decoding within 0.10 percentage points (the gate routes at most two of the 842 held-out queries at the selected $\tau^{*}$). Figure~\ref{fig:sound} shows the sweep for the structurally sound architectures: Idefics2-8B does not differ from either regime under an exact paired McNemar test ($p = 0.07$), and LLaVA-1.5-7B recovers its baseline at $\tau^{*} = 0.70$. At the selected operating points the output matches the baseline whenever no conflict is present, so the contrastive pass is conditional rather than mandatory.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig6.png}
\caption{The same sweep for the three structurally sound models, where EGCD recovers the greedy baseline at sufficiently high thresholds for the two LLaVA models while Universal CD+APC falls up to 10 points below it.}\label{fig:sound}
\end{figure}
...
\caption{Gain over greedy decoding across the entropy-threshold sweep for all seven models: every curve returns to zero as $\tau$ grows, and on the sound models a loose threshold costs up to 10 points.}\label{fig:all}
\end{figure}
...
\toprule
$\tau$ & \textbf{\shortstack{Qwen2-VL\\2B}} & \textbf{\shortstack{Phi-3.5-Vision\\4.2B}} & \textbf{\shortstack{SmolVLM2\\2.2B}} & \textbf{\shortstack{Qwen2.5-VL\\7B}} & \textbf{\shortstack{Idefics2\\8B}} & \textbf{\shortstack{LLaVA-1.5\\7B}} & \textbf{\shortstack{LLaVA-v1.6\\Mistral-7B}}\\
\midrule
